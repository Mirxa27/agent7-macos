...
# Optional dependency: browser-use
try:
    ...
    logging.getLogger(__name__).warning(
        "browser-use not installed. SessionManager will be unavailable. "
...
class SessionManager:
    ...
    def __init__(self, config: Optional[AgentBrowserConfig] = None):
        if not BROWSER_USE_AVAILABLE:
            raise RuntimeError(
                "SessionManager requires browser-use which is not installed. "
    ...
    async def initialize(self):
        ...
        if not BROWSER_USE_AVAILABLE or Browser is None:
