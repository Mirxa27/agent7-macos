...
try:
    ...
    logging.getLogger(__name__).warning(
        "OpenCV (cv2) and/or numpy not installed. "
        "Screenshot annotation will be unavailable. "
...
class SmartVision:
    ...
    def annotate_screenshot(
        ...
        if not self._cv2_available or cv2 is None or np is None:
            logger.warning(
                "OpenCV not available for screenshot annotation. "
                "Install with: pip install opencv-python numpy"
            )
